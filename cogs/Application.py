...
class RequirementView(discord.ui.View):
    options = [
        discord.SelectOption(label='Moderator', description='Open', emoji='👮'),
        discord.SelectOption(label='Developer', description='Closed', emoji='<:python:1259196508906197054>'),
        discord.SelectOption(label='Creator', description='Open', emoji='📣'),
        discord.SelectOption(label='Partner', description='Open', emoji='🤝')
    ]
    ...
    async def select_callback(self, select, interaction):
        ...
        if position == 'Moderator':
            ...
            req = "* Min. 16y/o\n* Basics of Progamming\n* One of the following languages: Python or " \
...
class ApplicationModal(discord.ui.Modal):
    ...
    async def callback(self, interaction):
        ...
        await channel.send(f"Application by: {interaction.user.mention} / For: {self.children[0].value}",
                           embed=embed_personal_statement)
